...
```bash
git clone https://github.com/kylegrahammatzen/harvest.git
cd harvest
bun install
bun setup
```

`bun setup` starts Redis via Docker, generates an encryption key, and creates your `.env` file. Fill in your Slack and Autumn credentials, then run `bun dev` and expose it with a tunnel like `ngrok http 3000` so Slack can reach your local server.
